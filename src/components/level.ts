...
const createLevelElement = (
  ...
  const levelElement = createElement(
    ...
    'level',
    ...[,],
    renderLevel.bind(null, containerElement, Nonogram.decode(boardHash))
  );
  ...
  levelElement.appendChild(
    createElement('div', 'level__bestTime', `${bestTime.toString()}s`)
  );

  levelElement.appendChild(
    createButtonElement('DELETE', () => {
